...
foreach ($product_tables as $table_name) {
    ...
    generateModel($product_db, $table_name, $test_mode, 'product');
}

foreach ($customer_tables as $table_name) {
    echo 'Making model for table ' . $table_name . PHP_EOL;
    generateModel($customer_db, $table_name, $test_mode, 'customer');
}

function generateModel($db, $table_name, $test_mode, $conn_name) {
    ...
    $model = '<?php namespace Pikd\Models;
...
class ' . $model_name . ' extends Model {
    ...
    protected $connection = \'' . $conn_name . '\';
